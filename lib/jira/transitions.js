  ...
  var transitions = {
    ...
    makeTransition: function (issue, value, cb) {
	    transitions.getTransitions(issue, function(err, transitionsAvailable){
          ...
          // Start from the first chain step whose id is in the current issue's
          // available transitions; skip earlier steps that are not offered now.
          // (Walk in sequence; do not jump to a later matching id.)
          ...
          for (var chainIndex = 0; chainIndex < transitionsArr.length; chainIndex++) {
            if (availableTransitionIds.indexOf(transitionsArr[chainIndex]) > -1) {
              startTransitionsFromIndex = chainIndex;
              break;
            }
          }
