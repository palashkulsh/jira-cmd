...
define([
  ...
  '../../lib/cache',
  '../../lib/auth'
], function (program, request, Table, config, cache, Auth) {
  ...
  var displaySprints = function displaySprints(rapidBoardID, sprintID, data, cb) {
    ...
    if (table.length == 0) {
      ...
      console.log("\nMatching Sprints:");
      console.log("=========================================\n")
      //just showing last 4 sprints rather than showing all sprints till now
      let disp_tble=new Table({head: ['Key', 'Name']})
      disp_tble.push.apply(disp_tble, table.slice(table.length-6))
      console.log(disp_tble.toString());
