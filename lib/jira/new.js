...
define([
  ...
  'async',
  'url'
], function (program, request, config, cache, async, url) {
  ...
  var create = {
    ...
    getMeta: function (key, callback) {
      ...
      request
        ...
        .end(function (res) {
          ...
          cache.set('meta', key?'project'+key:'project', res.body.values);       
    ...
    getProjectIssueFields: function ( callback) {
      this.query = `rest/api/2/issue/createmeta/MDO/issuetypes/3`;
      debugger
      var cachedRes = cache.getSync('meta', 'project', 1000*60*60*24*7);
      if(cachedRes){
        return callback(null, cachedRes);
    ...
    create: function(options, cb){      
      ...
      async.waterfall([
        function(wcb){
          create.getProject(function (err ,meta) {            
            if(err){
              return wcb(err);
            }
        ...
        function (options, wcb){
          create.getMeta(options.project, function (err ,meta) {            
            ...
            create.projectMeta.issuetypes = meta;
